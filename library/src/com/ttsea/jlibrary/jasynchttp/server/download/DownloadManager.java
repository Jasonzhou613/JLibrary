...
public class DownloadManager {
    ...
    private final int DEFAULT_MAX_DOWNLOADING_COUNT = 3;

    /** 同时最大下载数 */
    private int maxDownlodingCount = DEFAULT_MAX_DOWNLOADING_COUNT;

    /** 设置同时最大下载数，建议1-3 */
    public void setMaxDownlodingCount(int maxDownlodingCount) {
        if (maxDownlodingCount < 1) {
            throw new IllegalArgumentException("maxDownloadingCount must greater than 0.");
        }
        this.maxDownlodingCount = maxDownlodingCount;
    }

    public int getMaxDownlodingCount() {
        return maxDownlodingCount;
    }
        ...
        if (downloaderList != null) {
            for (int i = 0; i < downloaderList.size(); i++) {
                ...
                if (loader.getStatus() == Downloader.STATUS_RUNNING
                        || loader.getStatus() == Downloader.STATUS_PENDING) {
                    loader.pause(reason, false);
                }
            }
        }
    }
     ...
     * 并且控制同时下载的数量不超过maxDownlodingCount
     */
    public void fitDowloadList() {
        if (maxDownlodingCount <= getDownloadingCount()) {
            return;
        }
        for (int i = 0; i < downloaderList.size(); i++) {
            Downloader loader = downloaderList.get(i);
